	...
	public function __construct(){
	  ...
	  public function addNSubCatTitle($data){
		...
		if (empty($sub_category_title) || empty($sub_category_url) || empty($subcategory_seo_title)){
			...
			$ncatinsertquery = "INSERT INTO tbl_subcategory_Local(category_id,sub_category_title,sub_category_url,subcategory_seo_title,status) VALUES('$category_id','$sub_category_title','$sub_category_url','$subcategory_seo_title','$status') ";
	  ...
	  public function getAllNsCat(){
	  	$nscatShowquery = "SELECT * FROM tbl_subcategory_Local ORDER BY subcategory_id DESC";
		$result = $this->db->select($nscatShowquery);
		return $result;
	  }
	  public function gesubCategorybyId($subcid){
		$catShowquery = "SELECT * FROM tbl_subcategory_Local WHERE category_id = '$subcid' ";
		$result = $this->db->select($catShowquery);
		return $result;
	}

	  public function delNsCatById($id){
	  	$query = "DELETE FROM tbl_subcategory_Local WHERE subcategory_id = '$id'";
	  ...
	  public function changeNsCatStatusById($id){
	  	$query = "UPDATE tbl_subcategory_Local SET status = !status WHERE subcategory_id = '$id'";
		$changstutus = $this->db->update($query);
		return $changstutus;
	  }
	  public function getsubcatById($id){
	  	$nscatshow = "SELECT * FROM tbl_subcategory_Local WHERE subcategory_id = '$id'";
	  ...
	  public function editNSubCatTitle($data, $id){
		...
		if (empty($sub_category_title) || empty($sub_category_url) || empty($subcategory_seo_title)){
			...
			$updatequery = "UPDATE tbl_subcategory_Local 
							SET 
